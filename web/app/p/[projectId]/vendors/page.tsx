...
type TabKey = 'gates' | 'safety' | 'financial' | 'performance';
...
export default function VendorsPage() {
  ...
  const schedule = data.policy?.schedule_requirement;

  const openIssues = active
    ? active.gates.filter((g) => g.status !== 'pass').length
    : 0;

  const tabs: TabDef<TabKey>[] = [
    { key: 'gates', label: 'Gates', count: openIssues || undefined },
    { key: 'safety', label: 'Safety' },
    { key: 'financial', label: 'Bonding & insurance' },
    { key: 'performance', label: 'Performance & schedule' },
  ];

  return (
    <>
      <SectionHead eyebrow="Configuration" title="Prequalification">
        Whether a subcontractor may be carried on this package at all. These are gates, not weights:
        a failing gate removes a bidder from award consideration before price is discussed, because
        carrying that risk is not something a lower number buys back.
      </SectionHead>
      ...
      {/* ---------- bidder selector: the primary control, always visible ---------- */}
      ...
      {active && (
        <>
            ...
            <div className="small muted num" style={{ marginBottom: 8 }}>
              {activeVendor && (
                <>
                  ...
                  {money(activeVendor.adjusted_total)} leveled · rank {activeVendor.adjusted_rank} of{' '}
                  {vendors.length}
                </>
              )}
          ...
          <TabStrip tabs={tabs} active={tab} onChange={setTab} label="Prequalification sections" />

          {tab === 'gates' && (
            <TabPanel tabKey="gates">
              ...
              <p className="small muted" style={{ marginTop: 12 }}>
                Every bidder runs all six gates. A rule that did not fire still ran, so silence here
            ...
            <TabPanel tabKey="safety">
              <div className="card card-pad">
                ...
                <p className="small muted" style={{ marginTop: 16, marginBottom: 0 }}>
                  An EMR above the maximum is a gate, not a weight. It removes a bidder before price
                  is discussed, because carrying that risk is not something a lower number buys back.
                </p>
              </div>
            </TabPanel>
          )}
                ...
                <div className="card card-pad" style={{ flex: '1 1 320px' }}>
                  ...
                  <table>
                    <tbody>
                      ...
                      <tr>
                        {/* Backlog alone. The aggregate gate reports the same
                            ratio with this bid added, a slightly higher number --
                            both are labeled so the difference reads as intent. */}
                        <td>Aggregate utilization, before this bid</td>
                        <td
                          className="num"
                          style={{
                            textAlign: 'right',
                            color: active.bond_utilization_pct > 85 ? 'var(--warn)' : 'inherit',
                          }}
                        >
            ...
            <TabPanel tabKey="performance">
              <div className="row row-wrap" style={{ gap: 16, alignItems: 'stretch' }}>
                ...
                <div className="card card-pad" style={{ flex: '1 1 320px' }}>
                  ...
                  {schedule && (
                    <div className="small muted" style={{ marginBottom: 10 }}>
                      Package requires {schedule.required_duration_weeks} weeks, mobilizing within{' '}
                      {schedule.max_mobilization_weeks}
                    </div>
                  )}
                  <table>
                    <tbody>
                      <tr>
                        ...
                        <td
                          ...
                          style={{
                            ...
                            color:
                              schedule &&
                              active.schedule.proposed_duration_weeks > schedule.required_duration_weeks
                                ? 'var(--warn)'
                                : 'inherit',
                          }}
                        >
